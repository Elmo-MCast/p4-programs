...
header_type elmo_spine_complete_bitmap_t {
    fields {
        bitmap : 4; // NUM_LEAFS_PER_SPINE + NUM_CORES_PER_SPINE
    }
}
...
parser parse_elmo_type {
    ...
    return select(latest.type_) {
        ...
        default : parse_elmo_downstream_spine_head_p_rule;
    }
}
...
metadata elmo_downstream_spine_matching_bitmap_t elmo_downstream_spine_matching_bitmap;
...
parser parse_elmo_downstream_spine_head_p_rule {
    ...
    return select(latest.next_id, latest.next_p_rule, latest.has_default_bitmap, elmo_type.type_,
        ...
        SWITCH_ID mask 0x07 : parse_elmo_downstream_spine_head_p_rule_read_matching_bitmap;
        0x40 mask 0x40      : parse_elmo_downstream_spine_head_p_rule_last_id;
        0x20 mask 0x20      : parse_elmo_downstream_spine_last_p_rule;
        0x10 mask 0x10      : parse_elmo_downstream_spine_default_bitmap;
        default             : parse_elmo_downstream_leaf_head_p_rule;
    }
}

parser parse_elmo_downstream_spine_head_p_rule_read_matching_bitmap {
    set_metadata(elmo_downstream_spine_matching_bitmap.bitmap, elmo_downstream_spine_head_p_rule.bitmap);
    set_metadata(elmo_downstream_spine_matching_bitmap.is_matched, 1);
    return select(elmo_downstream_spine_head_p_rule.next_id, elmo_downstream_spine_head_p_rule.next_p_rule, elmo_downstream_spine_head_p_rule.has_default_bitmap) {
...
parser parse_elmo_downstream_spine_head_p_rule_last_id {
    ...
    return select(elmo_downstream_spine_head_p_rule.next_p_rule, elmo_downstream_spine_head_p_rule.has_default_bitmap, elmo_type.type_,
                  latest.id) { // Assuming LEAF_ID_SIZE_in_BITS = 12
        SWITCH_ID mask 0x07 : parse_elmo_downstream_spine_head_p_rule_last_id_read_matching_bitmap;
        0x20 mask 0x20      : parse_elmo_downstream_spine_last_p_rule;
        0x10 mask 0x10      : parse_elmo_downstream_spine_default_bitmap;
        default             : parse_elmo_downstream_leaf_head_p_rule;
    }
...
parser parse_elmo_downstream_spine_head_p_rule_last_id_read_matching_bitmap {
    set_metadata(elmo_downstream_spine_matching_bitmap.bitmap, elmo_downstream_spine_head_p_rule.bitmap);
    set_metadata(elmo_downstream_spine_matching_bitmap.is_matched, 1);
...
parser parse_elmo_downstream_spine_last_p_rule {
    ...
    return select(latest.next_id, elmo_downstream_spine_head_p_rule.has_default_bitmap, elmo_type.type_,
                  latest.id) { // Assuming LEAF_ID_SIZE_in_BITS = 12
        SWITCH_ID mask 0x07 : parse_elmo_downstream_spine_last_p_rule_read_matching_bitmap;
        0x20 mask 0x20      : parse_elmo_downstream_spine_last_p_rule_last_id;
        0x10 mask 0x10      : parse_elmo_downstream_spine_default_bitmap;
        default             : parse_elmo_downstream_leaf_head_p_rule;
    }
}

parser parse_elmo_downstream_spine_last_p_rule_read_matching_bitmap {
    set_metadata(elmo_downstream_spine_matching_bitmap.bitmap, elmo_downstream_spine_last_p_rule.bitmap);
    set_metadata(elmo_downstream_spine_matching_bitmap.is_matched, 1);
    return select(elmo_downstream_spine_last_p_rule.next_id, elmo_downstream_spine_head_p_rule.has_default_bitmap) {
...
parser parse_elmo_downstream_spine_last_p_rule_last_id {
    ...
    return select(elmo_downstream_spine_head_p_rule.has_default_bitmap, elmo_type.type_,
                  latest.id) { // Assuming LEAF_ID_SIZE_in_BITS = 12
        SWITCH_ID mask 0x07 : parse_elmo_downstream_spine_last_p_rule_last_id_read_matching_bitmap;
        1                   : parse_elmo_downstream_spine_default_bitmap;
        default             : parse_elmo_downstream_leaf_head_p_rule;
    }
}

parser parse_elmo_downstream_spine_last_p_rule_last_id_read_matching_bitmap {
    set_metadata(elmo_downstream_spine_matching_bitmap.bitmap, elmo_downstream_spine_last_p_rule.bitmap);
    set_metadata(elmo_downstream_spine_matching_bitmap.is_matched, 1);
...
parser parse_elmo_downstream_leaf_last_p_rule {
    ...
    return select(latest.next_id, elmo_downstream_leaf_head_p_rule.has_default_bitmap) {
...
metadata elmo_spine_complete_bitmap_t elmo_spine_complete_bitmap;

action elmo_upstream_spine_p_rule_act() {
    shift_left(elmo_spine_complete_bitmap.bitmap, elmo_upstream_spine_p_rule.downstream_bitmap, NUM_CORES_PER_SPINE);
    bit_or(elmo_spine_complete_bitmap.bitmap, elmo_spine_complete_bitmap.bitmap, elmo_upstream_spine_p_rule.upstream_bitmap);
    bitmap_port_select(elmo_spine_complete_bitmap.bitmap, elmo_upstream_spine_p_rule.multipath);
}

table elmo_upstream_spine_p_rule_tbl {
    actions {
        elmo_upstream_spine_p_rule_act;
    }
    size : 1;
}

action elmo_downstream_spine_p_rule_act() {
    shift_left(elmo_spine_complete_bitmap.bitmap, elmo_downstream_spine_matching_bitmap.bitmap, NUM_CORES_PER_SPINE);
    bitmap_port_select(elmo_spine_complete_bitmap.bitmap, 0);
}

table elmo_downstream_spine_p_rule_tbl {
    actions {
        elmo_downstream_spine_p_rule_act;
    }
    size : 1;
}

action elmo_downstream_spine_s_rule_act(mcast_group) {
    modify_field(intrinsic_metadata.mcast_grp, mcast_group);
}

table elmo_downstream_spine_s_rule_tbl {
    reads {
        inner_ipv4.dstAddr : exact;  // the group identifier
    }
    actions {
        elmo_downstream_spine_s_rule_act;
    }
    size : 10000;
}

action elmo_downstream_spine_default_p_rule_act() {
    shift_left(elmo_spine_complete_bitmap.bitmap, elmo_downstream_spine_default_bitmap.bitmap, NUM_CORES_PER_SPINE);
    bitmap_port_select(elmo_spine_complete_bitmap.bitmap, 0);
}

table elmo_downstream_spine_default_p_rule_tbl {
    actions {
        elmo_downstream_spine_default_p_rule_act;
    }
    size : 1;
}
...
control ingress {
    if (valid(elmo_type)) {
        if (valid(elmo_upstream_spine_p_rule)) {
            apply(elmo_upstream_spine_p_rule_tbl);
        } else {
            if (elmo_downstream_spine_matching_bitmap.is_matched == 1) {
              apply(elmo_downstream_spine_p_rule_tbl);
            } else {
                apply(elmo_downstream_spine_s_rule_tbl) {
                    miss {
                        apply(elmo_downstream_spine_default_p_rule_tbl);
                    }
                }
            }
        }
...
action elmo_invalidate_spine_p_rules_when_going_upstream_act() {
    remove_header(elmo_upstream_spine_p_rule);
    modify_field(elmo_type.type_, 0);
}

action elmo_invalidate_spine_p_rules_when_going_downstream_act() {
    remove_header(elmo_upstream_spine_p_rule);
...
table elmo_invalidate_spine_p_rules_tbl { // popping p-rules based on egress port
    reads {
        standard_metadata.egress_port: exact;
    }
    actions {
        elmo_invalidate_spine_p_rules_when_going_upstream_act;
        elmo_invalidate_spine_p_rules_when_going_downstream_act;
    }
...
control egress {
    if (valid(elmo_type)) {
        apply(elmo_invalidate_spine_p_rules_tbl);
    }
}
